...
class PDFGenerator:
    ...
    def generate_assembler_tasks(self, user_id):
        ...
        user_info = Database.fetch_one(
            "SELECT фио FROM Сотрудник WHERE id_сотрудника = %s", (user_id,)
        )
        ...
        query = """
            ...
            JOIN СоставЗаказа sz ON o.id_заказа = sz.id_заказа -- Приблизительная связь для инфо
            JOIN Изделие m ON sz.id_изделия = m.id_изделия
    ...
    def generate_assembler_schedule(self, user_id):
        ...
        query = """
            SELECT дата, статус FROM График 
