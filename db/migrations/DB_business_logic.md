...
15. Функция `sp_get_order_tasks(p_order_id INTEGER)`: Возвращает все производственные задачи (заготовки и сборки), созданные для указанного заказа. Используется для отображения задач в UI после создания заказа.

16. Таблица `план_сборки`: Хранит задачи на сборку изделий (аналогично `план_заготовок` для заготовок). Связана с заказами и изделиями.

17. Представление `v_задачи_сборщика` (обновлено): Объединяет задачи из `план_заготовок` (тип "заготовка") и `план_сборки` (тип "сборка") в единый набор для отображения в UI.


Примечания и замечания:
- Основной файл с актуальными процедурами: `consolidated_final.sql`
- Устаревшие миграции перенесены в папку `archive/`
- Функция `sp_add_order_item` теперь создаёт как задачи на сборку (`план_сборки`), так и задачи на заготовки (`план_заготовок`)
- Перед созданием заданий выполняется проверка материалов. Если материалов недостаточно, возвращается ошибка "НЕОБХОДИМА ЗАКУПКА"
