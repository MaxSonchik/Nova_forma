-- db/migrations/fix_warehouse_summary_cols.sql
...
CREATE OR REPLACE FUNCTION sp_get_warehouse_summary(
        ...
        SELECT id_материала as id_объекта,
        ...
        FROM Материал m
        UNION ALL
        SELECT id_заготовки as id_объекта,
        ...
        FROM Заготовка z
        UNION ALL
        SELECT id_изделия as id_объекта,
            'Изделие'::VARCHAR as тип,
            p.наименование,
            p.расчетный_остаток as количество,
            'шт'::VARCHAR as ед_изм
