...
\restrict jOeWSlbW1pclFxCltWtFzERYdNgOwGh8yoejic9yPxmS7C6in1CKOv3igHFAfzF
...
RETURN;
END IF;
-- Check if already assigned (optional warning, but we proceed to re-assign)
...
SET id_сотрудника = p_worker_id,
    статус = 'в_работе' -- CHANGED from 'назначено' (invalid) to 'в_работе' (valid)
WHERE id_заготовки = p_id_заготовки
    AND id_заказа = p_id_заказа;
RETURN NEXT;
EXCEPTION
...
CREATE TABLE public."ПланЗаготовок" (
    ...
    CONSTRAINT "план_заготовок_статус_check" CHECK ((("статус")::text = ANY ((ARRAY['принято'::character varying, 'в_работе'::character varying, 'выполнено'::character varying, 'отменено'::character varying, 'просрочено'::character varying])::text[])))
);
...
\unrestrict jOeWSlbW1pclFxCltWtFzERYdNgOwGh8yoejic9yPxmS7C6in1CKOv3igHFAfzF
