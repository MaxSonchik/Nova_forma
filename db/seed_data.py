...
def seed():
    ...
    tables = [
        "план_заготовок", "состав_закупки", "закупки_материалов",
        "график_работы", "состав_заказа", "состав_изделия", "расход_материалов", 
        "заказы", "изделия", "заготовки", "материалы", "клиенты", "сотрудники"
    ]
    for table in tables:
    ...
    base_materials = [
        ("ДСП Белый", "лист", 1500), ("ДСП Дуб Сонома", "лист", 1800), ("ДСП Венге", "лист", 1900),
        ("ДСП Орех", "лист", 1750), ("МДФ Глянец", "лист", 3500), ("Фанера 10мм", "лист", 900),
        ("Брус сосновый 50х50", "м", 120), ("Кромка ПВХ 2мм", "м", 40), ("Кромка Меламин", "м", 15),
        ("Саморезы 3.5х16", "шт", 1), ("Саморезы 4х30", "шт", 2), ("Евровинт (Конфирмат)", "шт", 5),
        ("Эксцентриковая стяжка", "шт", 15), ("Шкант деревянный", "шт", 2), ("Клей ПВА Столяр", "л", 450),
        ("Лак Полиуретановый", "л", 1200), ("Краска Акриловая", "л", 900), ("Ручка Скоба хром", "шт", 150),
        ("Ручка Кнопка", "шт", 80), ("Петля накладная с доводчиком", "шт", 180), ("Петля вкладная", "шт", 120),
        ("Направляющие шариковые 450мм", "компл", 400), ("Опора колесная", "шт", 120), ("Труба Джокер 25мм", "м", 300)
    ]
    ...
    zagotovki_names = [
        "Боковина шкафа 2200х600", "Полка 568х500", "Дверь шкафа ЛДСП", "Цоколь 600х100",
        "Крышка стола 1200х700", "Ножка стола (брус)", "Царга стола", "Спинка стула",
        "Сиденье стула", "Фасад ящика МДФ", "Дно ящика ДВП", "Задняя стенка ДВП"
    ]
    zagotovki_ids = []
    ...
    for z_name in zagotovki_names:
        ...
        """, (fake.unique.ean8(), z_name, random.randint(0, 50), fake.text(max_nb_chars=50)))
        z_id = cur.fetchone()[0]
    ...
    products_base = [
        ("Шкаф-купе 'Лидер'", "шкаф"), ("Стол письменный 'Ученик'", "стол"), ("Стул 'Комфорт'", "стул"),
        ("Тумба прикроватная", "шкаф"), ("Комод на 4 ящика", "шкаф"), ("Стол обеденный раздвижной", "стол"),
        ("Полка навесная", "шкаф"), ("Стеллаж офисный", "шкаф")
    ]
    product_ids = []
    ...
    for p_name, p_type in products_base:
        ...
        """, (
            fake.unique.ean8(), p_name, p_type, 
    ...
    # --- 4. СОТРУДНИКИ (объединены с users) ---
    print("🌱 4. Генерация сотрудников с доступами...")
    roles = ['сборщик', 'менеджер', 'директор']
    employees_ids = []
    
    # Создадим фиксированных пользователей для теста
    test_users = [
        ("Директор Иван", "director", "директор"), # Просто для теста логина
        ("Менеджер Анна", "manager", "менеджер"),
        ("Сборщик Петр", "worker", "сборщик")
    ]
    
    # 15 сотрудников всего
    for i in range(15):
        if i < 3:
            # Создаем фиксированных для удобства проверки
            fio, login_name, role = test_users[i]
            if role == "директор": role = "директор" # fix override
        else:
            ...
            # Безликий логин
            login_name = f"user_{random.randint(10000, 99999)}"
    ...
    # --- 5. ГРАФИК РАБОТЫ ---
    print("🌱 5. Заполнение графика работы...")
    today = date.today()
    statuses = ['рабочий', 'выходной', 'отпуск', 'больничный']
    # Генерируем график на 30 дней вперед для каждого сотрудника
    for emp_id in employees_ids:
        for day_offset in range(30):
            current_day = today + timedelta(days=day_offset)
            # 5/2 график примерно
            is_weekend = current_day.weekday() >= 5
            status = 'выходной' if is_weekend else 'рабочий'
            # Иногда случайный отгул
            if random.random() < 0.05: status = 'больничный'
    ...
    for _ in range(120):
        ...
        cur.execute("UPDATE заказы SET сумма_заказа = %s WHERE id_заказа = %s", (total_sum, o_id))
    ...
    for o_id in order_ids[:50]: # Для половины заказов создадим план
        ...
        cur.execute("SELECT id_изделия, количество_изделий FROM состав_заказа WHERE id_заказа = %s", (o_id,))
        order_items = cur.fetchall()
        
        for p_id, p_qty in order_items:
            # Получаем заготовки для изделия
            cur.execute("SELECT id_заготовки, количество_заготовок FROM состав_изделия WHERE id_изделия = %s", (p_id,))
            parts = cur.fetchall()
    ...
    suppliers = ["ООО 'ЛесСнаб'", "ИП Петров (Фурнитура)", "ЗАО 'ХимПродукт'", "МеталлКомплект"]
    ...
    for _ in range(20):
        ...
        for m_id in bought_mats:
            cur.execute("SELECT цена_за_единицу FROM материалы WHERE id_материала = %s", (m_id,))
