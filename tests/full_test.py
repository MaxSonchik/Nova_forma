...
class TestStage3Logic(unittest.TestCase):
    ...
    def test_03_director_procurement_logic(self):
        ...
        self.cur.execute(
            "SELECT id_материала, количество_на_складе FROM материалы LIMIT 1"
        )
        mat_id, initial_stock = self.cur.fetchone()

        # 2. Создаем закупку
        self.cur.execute(
            "INSERT INTO закупки_материалов (поставщик, статус) VALUES ('Тест Поставщик', 'подтверждено') RETURNING id_закупки"
        )
        ...
        # 5. Проверяем остаток
        self.cur.execute(
            "SELECT количество_на_складе FROM материалы WHERE id_материала = %s",
            (mat_id,),
        )
    ...
    def test_04_manager_auto_sum_trigger(self):
        ...
        self.cur.execute(
            "INSERT INTO заказы (id_клиента) VALUES (%s) RETURNING id_заказа", (c_id,)
        )
        ...
        self.cur.execute(
            "SELECT сумма_заказа FROM заказы WHERE id_заказа = %s", (o_id,)
        )
    ...
    def test_05_manager_production_plan_logic(self):
        ...
        self.cur.execute(
            "INSERT INTO заказы (id_клиента, дата_готовности) VALUES (%s, CURRENT_DATE + 10) RETURNING id_заказа",
            (c_id,),
        )
        o_id = self.cur.fetchone()[0]
        ...
        self.cur.execute(
            "SELECT count(*) FROM план_заготовок WHERE id_заказа = %s", (o_id,)
        )
        ...
        self.cur.execute(
            "SELECT id_плана FROM план_заготовок WHERE id_заказа = %s LIMIT 1", (o_id,)
        )
    ...
    def test_06_assembler_work_logic(self):
        ...
        self.cur.execute(
            "SELECT id_сотрудника FROM сотрудники WHERE должность='сборщик' LIMIT 1"
        )
        ...
        try:
            ...
            # 4. Проверяем статус
            self.cur.execute(
                "SELECT статус FROM план_заготовок WHERE id_плана = %s", (plan_id,)
            )
