...
class Toast(QWidget):
    ...
    def __init__(self, parent, title, message, theme_key):
        ...
        icon_label.setPixmap(qta.icon(theme["icon"], color=theme["icon_color"]).pixmap(32, 32))
        icon_label.setStyleSheet("background: transparent; border: none;") # Страховка
        layout.addWidget(icon_label)
        
        # Текст (Черный/Темный)
        msg_label = QLabel(f"<b style='font-size:14px; color:#2C3E50'>{title}</b><br><span style='font-size:13px; color:#404040'>{message}</span>")
