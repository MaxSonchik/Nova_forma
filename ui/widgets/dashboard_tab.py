from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QFrame, 
                             QGridLayout, QPushButton, QDateEdit, QGroupBox,
                             QGraphicsDropShadowEffect)
from PyQt6.QtCore import Qt, QDate, pyqtSignal, QPropertyAnimation, QEasingCurve, QObject
...
class ClickableCard(QFrame):
    ...
    def __init__(self, title, value, icon_name, color_hex):
        ...
        self.lbl_value.setStyleSheet("color: #2C3E50; font-size: 22px; font-weight: bold;")
...
class DashboardTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Ряд 1
        self.c_revenue = self.add_card(0, 0, "Выручка", "0 ₽", "fa5s.coins", "#27AE60", "revenue")
        self.c_expense = self.add_card(0, 1, "Расходы", "0 ₽", "fa5s.shopping-cart", "#E74C3C", "expenses")
        self.c_profit = self.add_card(0, 2, "Прибыль (расч.)", "0 ₽", "fa5s.chart-line", "#3498DB", "profit")
        self.c_orders = self.add_card(0, 3, "Заказов закрыто", "0", "fa5s.check-double", "#F39C12", "orders_count")
        
        # Ряд 2
        self.c_avg = self.add_card(1, 0, "Средний чек", "0 ₽", "fa5s.receipt", "#9B59B6", "avg_check")
        self.c_margin = self.add_card(1, 1, "Рентабельность", "0 %", "fa5s.percent", "#1ABC9C", "profit")
        self.c_cancel = self.add_card(1, 2, "Процент отмен", "0 %", "fa5s.ban", "#7F8C8D", "cancel_rate")
        self.c_active = self.add_card(1, 3, "Активных сотрудн.", "0", "fa5s.users", "#34495E", None)
    ...
    def load_data(self):
        ...
        """.replace("status", "статус"), (d_start, d_end)) # fix column name just in case
        ...
        # 4. Сотрудники
        staff_res = Database.fetch_one("SELECT COUNT(*) as cnt FROM сотрудники WHERE дата_увольнения IS NULL")
        staff = staff_res['cnt']
    ...
    def open_detail(self, title, metric_type):
        ...
        if not data:
            Toast.warning(self, "Нет данных", f"За период {d_start} - {d_end}\nнет статистики.")
            return

        dialog = DetailStatsDialog(self, title, metric_type, d_start, d_end, preloaded_data=data)
        dialog.exec()
