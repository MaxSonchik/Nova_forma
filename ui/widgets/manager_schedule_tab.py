...
class ManagerScheduleTab(QWidget):
    ...
    def load_employees(self):
        ...
        emps = Database.fetch_all("SELECT id_сотрудника, фио, должность FROM сотрудники ORDER BY фио")
        for e in emps:
    ...
    def apply_status_to_selection(self, status_code, status_name):
        ...
        try:
            for item in selected_items:
                ...
                Database.execute("CALL sp_установить_статус_дня(%s, %s, %s)", 
                               (emp_id, date_str, status_code))
                count += 1
            
            Toast.success(self, "Успешно", f"Статус '{status_name}' установлен\nдля {count} дн.")
            self.load_schedule_colors() 
