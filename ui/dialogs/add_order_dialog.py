...
class AddOrderDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.date_edit.setMinimumDate(QDate.currentDate().addDays(1)) # Блокируем прошлое
        ...
        self.table_cart.setHorizontalHeaderLabels(["Изделие", "Кол-во", "Цена/шт", "Сумма"])
        ...
        self.lbl_total.setStyleSheet("font-size: 16px; font-weight: bold; color: #2C3E50;")
    ...
    def load_dictionaries(self):
        ...
        # Изделия
        products = Database.fetch_all("SELECT id_изделия, наименование, стоимость, количество_на_складе FROM изделия ORDER BY наименование")
        for p in products:
            text = f"{p['наименование']} (Остаток: {p['количество_на_складе']}) - {p['стоимость']} ₽"
            # Храним кортеж (id, price) в userData
            self.combo_product.addItem(text, (p['id_изделия'], p['стоимость'], p['наименование']))
    ...
    def save_order(self):
        ...
        try:
            ...
            for item in self.cart_items:
                ...
                res_msg = Database.fetch_one("SELECT sp_добавить_изделие_в_заказ(%s, %s, %s)", 
