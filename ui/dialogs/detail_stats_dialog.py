...
class DetailStatsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Шапка
        top = QHBoxLayout()
        info = QLabel(f"<h2>{self.metric_title}</h2>Период: {self.d_from} — {self.d_to}")
    ...
    def export_pdf(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Сохранить", "Report.pdf", "PDF (*.pdf)")
        ...
        try:
            ...
            if os.path.exists(logo): c.drawImage(logo, 50, h - 100, 60, 60, mask='auto')
            
            c.drawString(130, h - 70, f"Отчет: {self.metric_title}")
            c.setFont('RusFont', 10)
            c.drawString(130, h - 90, f"Период: {self.d_from} - {self.d_to}")
            
            c.drawImage(img_path, 50, h - 500, width=500, height=350, preserveAspectRatio=True)
